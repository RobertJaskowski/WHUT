...
public class NovelController : MonoBehaviour
{
    ...
    private Dictionary<string, TextAsset> stories = new Dictionary<string, TextAsset>();
    ...
    void Awake()
    {
        ...
        foreach(TextAsset e in texts)
        {
            stories.Add(e.name, e);
        }
    }
    ...
    public void LoadGameFile(string gameFileName)
    {
        ...
        string filePath = FileManager.savPath + "savData/gameFiles/" + activeGameFileName + ".txt";//if not working put game file name here
        ...
        stories.TryGetValue(activeGameFile.chapterName, out ass);

        data = FileManager.ReadTextAsset(ass);
        ...
        for (int i = 0; i < activeGameFile.charactersInScene.Count; i++)
        {
            ...
            Character character = CharacterManager.instance.CreateCharacter(data.characterName, "A", data.enabled);
    ...
    public void addChoiceToGameFile(CHOICE choice)
    {
        activeGameFile.choicesMade.Add(choice.key, choice);
    }

    public bool gameFileHasChoiceMade(string key)
    {
        bool result = activeGameFile.choicesMade.ContainsKey(key);
        return result;
    }
    ...
    public void LoadChapterFile(string fileName)
    {
        ...
        stories.TryGetValue(fileName,out ass);
    ...
    IEnumerator HandlingChoiceLine(string line)
    {
        ...
        while (gatheringChoices)
        {
            ...
            if (line != "}")
            {
                ...
                keys.Add(line.Split('"')[2]);
                actions.Add(data[chapterProgress + 1].Trim());//Replace("    ","")
                chapterProgress++;
            }
            else
            {
        ...
        if (choices.Count > 0)
        {
            ChoiceScreen.Show(keys.ToArray() ,title, choices.ToArray());
            yield return new WaitForEndOfFrame();
    ...
    public void HandleAction(string action)
    {
        ...
        switch (data[0])
        {
            ...
            case "setBox":
                Command_SetLayerImage(data[1], BCFC.instance.box);
                break;
            ...
            case "panel":
                Command_PanelVisibility(data[1]);
                break;

            case "showChapterScreen":
                Command_SetLayerImage(data[1], BCFC.instance.chapterLayer);
                break;

            case "decideEvent":
                Command_DecideEvent(data[1]);
                break;
    ...
    private void Command_ShowCredits()
    {
        creditsScreen.SetActive(true);
    }

    private void Command_DecideEvent(string data)
    {
        string key = data.Split(',')[0];
        string chapterName = data.Split(',')[1];
        

        if (gameFileHasChoiceMade(key))
        {
            Command_Load(chapterName);
        }
    }
    ...
    void Command_SetLayerImage(string data, BCFC.LAYER layer)
    {
        ...
        if (data.Contains(","))
        {
            ...
            spd = float.Parse(parameters[1], CultureInfo.InvariantCulture);
    ...
    void Command_MoveCharacter(string data)
    {
        ...
        float locationY = parameters.Length >= 3 ? float.Parse(parameters[2], CultureInfo.InvariantCulture) : 0;
        float speed = parameters.Length >= 4 ? float.Parse(parameters[3], CultureInfo.InvariantCulture) : 7f;
    ...
    void Command_SetFace(string data)
    {
        ...
        float speed = parameters.Length == 3 ? float.Parse(parameters[2], CultureInfo.InvariantCulture) : 3f;
    ...
    void Command_Enter(string data)
    {
        ...
        foreach (string s in characters)
        {
            Character c = CharacterManager.instance.GetCharacter(s, true, "A", false);
            if (!c.enabled)
            {
    ...
    void Command_PanelVisibility(string data)
    {
